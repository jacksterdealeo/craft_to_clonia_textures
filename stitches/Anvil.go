...
func RWAnvil(input_pack_path, output_pack_path string, _ *configure.Config) error {
	stitch := "Anvil"
	in_path := filepath.Join(input_pack_path, data.CraftPaths["block"])
	out_path := filepath.Join(output_pack_path, data.CloniaPaths["anvils"])

	abase, err := imaging.Open(filepath.Join(in_path, "anvil.png"))
	if err != nil {
		return openErrMsg(stitch, "block", "anvil.png")
	}
	a0, err := imaging.Open(filepath.Join(in_path, "anvil_top.png"))
	if err != nil {
		return openErrMsg(stitch, "block", "anvil_top.png")
	}
	a1, err := imaging.Open(filepath.Join(in_path, "chipped_anvil_top.png"))
	if err != nil {
		return openErrMsg(stitch, "block", "chipped_anvil_top.png")
	}
	a2, err := imaging.Open(filepath.Join(in_path, "damaged_anvil_top.png"))
	if err != nil {
	...
	if err = imaging.Save(new_a0, filepath.Join(out_path, "mcl_anvils_anvil_top_damaged_0.png")); err != nil {
		return saveErrMsg(stitch, "anvils", save_name)
	}
	if err = imaging.Save(new_a1, filepath.Join(out_path, "mcl_anvils_anvil_top_damaged_1.png")); err != nil {
		return saveErrMsg(stitch, "anvils", save_name)
	}
	if err = imaging.Save(new_a2, filepath.Join(out_path, "mcl_anvils_anvil_top_damaged_2.png")); err != nil {
		return saveErrMsg(stitch, "anvils", save_name)
	}
	return nil
}
