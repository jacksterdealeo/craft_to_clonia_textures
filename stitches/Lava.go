...
func RWLava(input_pack_path, output_pack_path string, _ *configure.Config) error {
	...
	/*
		...
		clonia lava
		  ...
		  flowing :  16 x 1024
	*/
	lavaFlowing, err := imaging.Open(filepath.Join(inPath, "lava_flow.png"))
	if err != nil {
		...
		if err = imaging.Save(dst, filepath.Join(voxPath, "mcl_core_lava_flow_animation.png")); err != nil {
			return saveErrMsg("Lava", "vl", "mcl_core_lava_flow_animation.png")
		}
		if err = imaging.Save(dst, filepath.Join(outPath, "default_lava_flowing_animated.png")); err != nil {
