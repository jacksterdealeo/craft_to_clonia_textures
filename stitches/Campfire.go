...
func RWCampfire(input_pack_path, output_pack_path string, _ *configure.Config) error {
	...
	out_path := filepath.Join(output_pack_path, data.CloniaPaths["campfires"])

	fails := []string{}

	campfire_log_lit, err := imaging.Open(filepath.Join(in_path, "campfire_log_lit.png"))
	if err != nil {
		...
		if err := imaging.Save(dst, filepath.Join(out_path, "mcl_campfires_campfire_log_lit.png")); err != nil {
			fails = append(fails, "mcl_campfires_campfire_log_lit.png failed to save!")
		}
	}

	soul_campfire_log_lit, err := imaging.Open(filepath.Join(in_path, "soul_campfire_log_lit.png"))
	if err != nil {
		...
		if err := imaging.Save(dst, filepath.Join(out_path, "mcl_campfires_soul_campfire_log_lit.png")); err != nil {
			fails = append(fails, "mcl_campfires_soul_campfire_log_lit.png failed to save!")
		}
	}

	campfire_log, err := imaging.Open(filepath.Join(in_path, "campfire_log.png"))
	if err != nil {
		...
		if err := imaging.Save(dst, filepath.Join(out_path, "mcl_campfires_log.png")); err != nil {
			fails = append(fails, "mcl_campfires_log.png failed to save!")
		}
	}

	fire, err := imaging.Open(filepath.Join(in_path, "campfire_fire.png"))
	if err != nil {
		fails = append(fails, "campfires::campfire_fire.png failed to open!")
	} else {
		dst := imaging.New(fire.Bounds().Dx()*2, fire.Bounds().Dy(), color.NRGBA{0, 0, 0, 0})
		dst = imaging.Paste(dst, fire, image.Pt(fire.Bounds().Dx(), 0))
		if err := imaging.Save(dst, filepath.Join(out_path, "mcl_campfires_campfire_fire.png")); err != nil {
			fails = append(fails, "mcl_campfires_campfire_fire.png failed to save!")
		}
	}

	soulfire, err := imaging.Open(filepath.Join(in_path, "soul_campfire_fire.png"))
	if err != nil {
		...
		if err := imaging.Save(dst, filepath.Join(out_path, "mcl_campfires_soul_campfire_fire.png")); err != nil {
